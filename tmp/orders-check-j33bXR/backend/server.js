...
async function ensureAdminAccount() {
  ...
  if (existing) {
    await pool.query(
      'UPDATE business_accounts SET business_name = ?, username = ?, password_hash = ?, owner_name = ?, email = ?, phone = ?, role = ?, status = ? WHERE id = ?',
      ['Apexiums HQ', adminUsername, passwordHash, 'Super Admin', 'admin@apexiums.com', '03000000000', 'SuperAdmin', 'Active', 1]
    );
    return;
  }
  await pool.query(
...
app.post('/api/customers/register', async (req, res) => {
  try {
    ...
    const customer = { id: result.insertId, name: req.body.name, username, email, phone, status: 'Active' };
    await createCustomerSession(req, res, customer);
