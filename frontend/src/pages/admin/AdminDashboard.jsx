...
const AdminDashboardContent = ({ session, storeName, logoSrc, onLogout }) => {
  const { activeTab, setActiveTab, hasPermission, setIsSearchOpen } =
    useAdmin();
  ...
  React.useEffect(() => {
    const slugToPage = Object.fromEntries(
      Object.entries(pageSlugs).map(([page, slug]) => [slug, page]),
    );
    const requestedPage =
      slugToPage[window.location.pathname.replace(/^\/+|\/+$/g, "")];
    if (requestedPage && requestedPage !== activeTab)
  ...
  React.useEffect(() => {
    ...
    if (window.location.pathname !== nextPath)
      window.history.pushState({ page: activeTab }, "", nextPath);
  }, [activeTab]);
  ...
  const renderActiveView = () => {
    const tabPermissions = {
      ...
      revenue: "manageFinance",
      expenses: "manageFinance",
      "software-fees": "manageFinance",
      "staff-salaries": "manageFinance",
      "delivery-expenses": "manageFinance",
      chats: "manageChats",
      notifications: "viewNotifications",
      settings: "manageSettings",
    };
    if (
      ["permissions", "business-accounts"].includes(activeTab) &&
    ...
    if (
      ...
      !hasPermission(tabPermissions[activeTab])
    ) {
      return (
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-8 text-center">
          <h2 className="text-lg font-black text-amber-800">Access denied</h2>
          <p className="mt-1 text-xs text-amber-700">
            Ask SuperAdmin to enable this permission for your role.
          </p>
        </div>
      );
    }
          ...
          {activeTab === "dashboard" && (
            <div className="flex items-center justify-between gap-4">
              ...
              >
                ...
                <span className="ml-auto rounded bg-slate-50 px-1.5 py-0.5 text-[10px] font-mono text-slate-400">
                  Ctrl+K
                </span>
              </button>
              <label
