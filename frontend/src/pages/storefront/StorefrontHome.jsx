...
export default function StorefrontHome({ onLogin, session, onLogout }) {
  ...
  const [applicationForm, setApplicationForm] = React.useState({ applicant_name: '', business_name: '', email: '', phone: '', address: '', category: '', proposed_amount: '', investment_product: '', document_url: '', message: '' });
          ...
          <div className={`w-full ${infoModal.type === 'investor-application' ? 'max-w-xl' : 'max-w-sm'} max-h-[92vh] overflow-y-auto rounded-3xl bg-white p-5 sm:p-6 shadow-2xl border border-slate-100 text-slate-900 animate-in fade-in zoom-in-95 duration-200`}>
            ...
            {infoModal.type ? <form className="mt-4 space-y-3" onSubmit={async (event) => {
              ...
              {infoModal.type === 'investor-application' ? <>
                ...
                <label className="block text-xs font-black text-slate-900">Upload ID or Document Picture<input required type="file" accept="image/*" onChange={(event) => { const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => setApplicationForm({ ...applicationForm, document_url: reader.result }); reader.readAsDataURL(file); }} className="mt-1.5 w-full rounded-2xl bg-slate-50 px-4 py-3 text-xs font-medium"/></label>
