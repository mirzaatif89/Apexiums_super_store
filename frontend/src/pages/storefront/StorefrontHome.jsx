...
export default function StorefrontHome({ onLogin, session, onLogout }) {
  ...
  const [applicationForm, setApplicationForm] = React.useState({ applicant_name: '', business_name: '', email: '', phone: '', address: '', category: '', proposed_amount: '', investment_product: '', document_url: '', document_name: '', confirm_information: false, accept_terms: false, message: '' });
    ...
    >
      ...
      {infoModal ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-xs">
          <div className={`w-full ${infoModal.type === 'investor-application' ? 'max-w-xl' : 'max-w-sm'} max-h-[92vh] overflow-y-auto rounded-3xl bg-white p-5 sm:p-6 shadow-2xl border border-slate-100 text-slate-900 animate-in fade-in zoom-in-95 duration-200`}>
            ...
            <p className={`mt-4 text-xs sm:text-sm leading-relaxed font-medium ${infoModal.type === 'investor-application' ? 'rounded-2xl bg-[#FF3D50] p-5 text-white' : 'text-slate-600'}`}>{infoModal.content}</p>
            ...
            {infoModal.type ? <form className="mt-4 space-y-3" onSubmit={async (event) => {
              ...
              const payload = infoModal.type === 'seller-application' ? { applicant_name: applicationForm.applicant_name, business_name: applicationForm.business_name, email: applicationForm.email, phone: applicationForm.phone, category: applicationForm.category, message: applicationForm.message, status: 'Pending' } : { applicant_name: applicationForm.applicant_name, email: applicationForm.email, phone: applicationForm.phone, address: applicationForm.address, proposed_amount: Number(applicationForm.proposed_amount || 0), investment_product: applicationForm.investment_product, document_url: applicationForm.document_url, message: applicationForm.message, status: 'Pending' };
              if (infoModal.type === 'investor-application' && (!applicationForm.confirm_information || !applicationForm.accept_terms)) { setApplicationStatus('Please confirm the information and accept the Terms and Conditions.'); return; }
              try { const response = await fetch(`/api/${resource}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); if (!response.ok) throw new Error('Application could not be submitted.'); setApplicationStatus('Application submitted successfully. Our team will review it.'); setApplicationForm({ applicant_name: '', business_name: '', email: '', phone: '', address: '', category: '', proposed_amount: '', investment_product: '', document_url: '', document_name: '', confirm_information: false, accept_terms: false, message: '' }); } catch (error) { setApplicationStatus(error.message); }
            }}>
              ...
              </> : <><div className="grid grid-cols-2 gap-2"><input required value={applicationForm.applicant_name} onChange={(event) => setApplicationForm({ ...applicationForm, applicant_name: event.target.value })} placeholder="Full name" className="rounded-xl border px-3 py-2 text-xs"/><input required type="email" value={applicationForm.email} onChange={(event) => setApplicationForm({ ...applicationForm, email: event.target.value })} placeholder="Email address" className="rounded-xl border px-3 py-2 text-xs"/></div><input required value={applicationForm.phone} onChange={(event) => setApplicationForm({ ...applicationForm, phone: event.target.value })} placeholder="Phone number" className="w-full rounded-xl border px-3 py-2 text-xs"/><div className="grid grid-cols-2 gap-2"><input required value={applicationForm.business_name} onChange={(event) => setApplicationForm({ ...applicationForm, business_name: event.target.value })} placeholder="Business name" className="rounded-xl border px-3 py-2 text-xs"/><input required value={applicationForm.category} onChange={(event) => setApplicationForm({ ...applicationForm, category: event.target.value })} placeholder="Product category" className="rounded-xl border px-3 py-2 text-xs"/></div><textarea rows="3" value={applicationForm.message} onChange={(event) => setApplicationForm({ ...applicationForm, message: event.target.value })} placeholder="Additional details" className="w-full rounded-xl border px-3 py-2 text-xs"/></>}
              {applicationStatus ? <p className="text-xs font-bold text-emerald-600">{applicationStatus}</p> : null}
              <button disabled={infoModal.type === 'investor-application' && (!applicationForm.confirm_information || !applicationForm.accept_terms || !applicationForm.document_url)} className="w-full rounded-xl bg-[#E8262A] py-3 text-xs font-bold text-white disabled:cursor-not-allowed disabled:bg-slate-300">Submit Form</button>
            </form> : null}
