...
export const AdminProvider = ({ children, session }) => {
  ...
  const reviewSellerApplication = async (application, decision) => {
    if (decision === 'Approved') {
      ...
      const response = await fetch('/api/wholesellers', { method: 'POST', headers: apiHeaders(), body: JSON.stringify({ name: application.applicant_name, business_name: application.business_name, contact_person: application.applicant_name, phone: application.phone, email: application.email, address: application.address, description: [application.leopard_courier_nearby ? `Leopard Courier nearby: ${application.leopard_courier_nearby}` : '', application.message || ''].filter(Boolean).join('\n'), seller_image: application.product_image_url, stock_seller_sell: application.category, username, password: `Seller@${application.id}2026`, status: 'Active' }) });
