...
export default function Header({
  ...
  return (
    <header
      className="relative z-20 w-full rounded-none text-white transition-all"
        ...
        <div className="block md:hidden space-y-3 sm:space-y-3.5">
          ...
          <div className="flex items-center justify-between gap-3 w-full">
            ...
            <button
              type="button"
              onClick={onCartClick}
              className="group relative flex h-10 w-10 sm:h-11 sm:w-11 items-center justify-center rounded-2xl bg-white/20 hover:bg-white/30 text-white shadow-xs transition active:scale-95 cursor-pointer shrink-0 backdrop-blur-xs"
              title="Cart"
              aria-label="Cart"
            >
              <ReferenceCartIcon className="h-[19px] w-[19px] text-white transition-transform group-hover:scale-105" />
              {cartCount > 0 ? (
                <span className="absolute -top-1 -right-1 flex h-4.5 min-w-4.5 items-center justify-center rounded-full bg-white px-1 text-[9px] font-black text-[#E8262A] shadow-md animate-bounce">
                  {cartCount}
                </span>
