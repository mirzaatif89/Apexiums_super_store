  ...
  return (
    <div className="space-y-6">
      ...
      <div className="overflow-hidden rounded-2xl border border-red-100 bg-white shadow-xs">
        <div className="border-b bg-red-50/60 p-4"><h3 className="text-sm font-black text-slate-900">Website Investor Applications</h3><p className="text-[11px] text-slate-500">Approve proposals to register applicants as active investors.</p></div>
        <div className="overflow-x-auto"><table className="w-full min-w-[950px] text-left text-xs"><thead><tr className="border-b bg-slate-50 text-[10px] font-bold uppercase text-slate-500"><th className="p-3">Applicant</th><th className="p-3">Contact & Address</th><th className="p-3">Proposed Amount</th><th className="p-3">Investment Product</th><th className="p-3">ID / Document</th><th className="p-3">Status</th><th className="p-3 text-right">Review</th></tr></thead><tbody className="divide-y">
          {!investorApplications.length ? <tr><td colSpan={7} className="p-8 text-center text-slate-400">No investor applications received yet.</td></tr> : investorApplications.map((application) => <tr key={application.id}><td className="p-3"><p className="font-bold">{application.applicant_name}</p><p className="max-w-xs truncate text-[10px] text-slate-400">{application.message || 'No additional details'}</p></td><td className="p-3"><p>{application.email}</p><p>{application.phone}</p><p className="max-w-xs truncate text-slate-400">{application.address || '-'}</p></td><td className="p-3 font-black text-red-600">Rs {Number(application.proposed_amount || 0).toLocaleString('en-PK')}</td><td className="p-3 font-bold">{application.investment_product || '-'}</td><td className="p-3">{application.document_url ? <a href={application.document_url} target="_blank" rel="noreferrer" className="font-bold text-blue-600 hover:underline">View Document</a> : '-'}</td><td className="p-3"><Badge status={application.status}>{application.status}</Badge></td><td className="p-3 text-right">{application.status === 'Pending' ? <div className="inline-flex gap-2"><button onClick={() => reviewInvestorApplication(application, 'Rejected')} className="rounded-lg border border-red-200 px-3 py-2 font-bold text-red-600">Reject</button><button onClick={() => reviewInvestorApplication(application, 'Approved')} className="rounded-lg bg-red-600 px-3 py-2 font-bold text-white">Approve & Register</button></div> : <span className="font-bold text-slate-500">Reviewed</span>}</td></tr>)}
        </tbody></table></div>
      </div>
