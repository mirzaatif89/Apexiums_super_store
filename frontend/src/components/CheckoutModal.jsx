...
export default function CheckoutModal({
  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(orderResult.message || 'Unable to place your order. Please try again.');
      const backendOrderId = orderResult?.order?.id || orderResult?.id;
      if (!backendOrderId) throw new Error('Order confirmation was not received. Please check your orders before retrying.');
