...
export const CategoriesView = () => {
  ...
  const handleOpenEdit = (c) => {
    ...
    setFormData({
      ...
      image: c.image,
      status: c.status,
      subcategories: Array.isArray(c.subcategories) ? c.subcategories : [], subcategoryDraft: ''
    });
  ...
  const handleSubmit = async (e) => {
    ...
    const formattedData = {
      ...formData,
      image: formData.image || uploadPreview || '',
            ...
            <div>
              <div className="relative h-36 w-full">
                {c.image ? (
                  ...
                  <div className="flex h-full w-full items-center justify-center bg-slate-200 text-5xl font-black text-slate-400">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col px-6 py-6 space-y-4 text-xs">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input type="text" value={formData.subcategoryDraft} onChange={(e) => setFormData({ ...formData, subcategoryDraft: e.target.value })} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); const value = formData.subcategoryDraft.trim(); if (value && !formData.subcategories.includes(value)) setFormData({ ...formData, subcategories: [...formData.subcategories, value], subcategoryDraft: '' }); } }} className="min-w-0 flex-1 px-3 py-2 border rounded-xl font-semibold focus:outline-none" placeholder="e.g. Cycling" />
              ...
              <div className="order-first">
                ...
                <input
                  type="file"
                  accept="image/*"
                  required={!editingCategory && !uploadPreview && !formData.image}
                  onChange={(e) => {
