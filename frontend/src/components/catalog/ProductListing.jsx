...
export const ProductListing = () => {
  ...
  const handleSaveProduct = async (e) => {
    e.preventDefault();
    if (!formData.name || !formData.category || !formData.subcategory || !formData.realPrice || !formData.discountedPrice || formData.stock === '') {
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-2xl w-full overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form noValidate onSubmit={handleSaveProduct} className="p-5 overflow-y-auto space-y-4">
              ...
              <div>
                ...
                <input type="file" accept="image/*" required={!editingProduct} onChange={(e) => { const file = e.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => setFormData({ ...formData, image: String(reader.result || '') }); reader.readAsDataURL(file); }} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-200" />
                {formData.image && <img src={formData.image} alt="Main product" className="mt-2 h-24 w-24 rounded-lg border object-cover" />}
              </div>
              <div>
                <label className="block font-bold text-slate-700 text-xs mb-1">Upload Product Subimages (up to 4)</label>
                <input type="file" accept="image/*" multiple onChange={(e) => { const files = Array.from(e.target.files || []).slice(0, 4); Promise.all(files.map((file) => new Promise((resolve) => { const reader = new FileReader(); reader.onload = () => resolve({ url: String(reader.result || ''), style: file.name.replace(/\.[^.]+$/, '') }); reader.readAsDataURL(file); }))).then((images) => setFormData({ ...formData, images })); }} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-200" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-slate-200 rounded-xl text-xs font-mono font-semibold focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-200"
                  />
                </div>
                ...
                <div>
                  ...
                  <select value={formData.subcategory} onChange={(e) => setFormData({ ...formData, subcategory: e.target.value })} className="w-full px-3 py-2 border border-slate-200 rounded-xl text-xs font-semibold focus:outline-none">
                    <option value="">Select subcategory</option>
                    {(categories.find((c) => c.name === formData.category)?.subcategories || []).map((sub) => <option key={sub} value={sub}>{sub}</option>)}
                  </select>
                </div>
              ...
              <div>
                <label className="block font-bold text-slate-700 text-xs mb-1">Product Subimages Preview</label>
                <input
                  type="file"
                  accept="image/*"
                  multiple
                  onChange={(e) => {
                    ...
                    Promise.all(files.map((file) => new Promise((resolve) => { const reader = new FileReader(); reader.onload = () => resolve({ url: String(reader.result || ''), style: file.name.replace(/\.[^.]+$/, '') }); reader.readAsDataURL(file); }))).then((images) => setFormData({ ...formData, images, image: images[0]?.url || '' }));
                  }}
                  className="w-full rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold focus:outline-none"
                />
                {formData.images?.length > 0 && <div className="grid grid-cols-5 gap-2 mt-2">{formData.images.map((item, index) => <div key={item.url}><img src={item.url} alt={item.style} className="h-16 w-full rounded-lg border object-cover" /><input value={item.style} onChange={(e) => setFormData({ ...formData, images: formData.images.map((imageItem, itemIndex) => itemIndex === index ? { ...imageItem, style: e.target.value } : imageItem) })} className="w-full mt-1 rounded border px-1 text-[9px]" placeholder="Style" /></div>)}</div>}
              </div>
